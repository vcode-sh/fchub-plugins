...
FCHub installs only from official FCHub releases on GitHub, and checks each download against its published checksum — where one is published — before handing anything to WordPress. Four things can go wrong, and none of them leave a half-installed plugin behind.
