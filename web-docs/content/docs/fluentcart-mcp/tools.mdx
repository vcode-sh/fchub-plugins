...
description: Every tool definition in FluentCart MCP, listed by module — orders, products, customers, subscriptions, shipping, tax, reports, and more.
---
...
The inventory of tool definitions shipped in this release, by module, with the HTTP method behind each one.

How many of them your AI can actually see is a different question, and the honest answer is: it depends. Two filters run before anything is registered — your write mode decides which risk classes may exist, and your store's discovered routes remove every tool it cannot serve. Your WordPress user's permissions then decide which of the survivors succeed. No fixed number can describe that, so this page doesn't print one — `release-contract.json` in the repository carries the measured figures for each configuration at a known revision, which is the only place a count means anything.

You'll never call these yourself — the AI picks the right tool from what you ask. This page is for confirming that something you want to do is possible before you ask for it.
...
Before any mode registers anything, the write policy decides what may exist:

| `FLUENTCART_WRITE_MODE` | What's in the registry |
|---|---|
| `disabled` *(the default)* | Read-only tools |
| `reversible` | Reads plus writes you can undo |
| `guarded` | The same set as `reversible` in 2.0.0. It is the mode the money tools will return to; today it adds nothing |

`reversible` is the read set plus the writes that can be undone — creates and updates, nothing more. Deletion, bulk operations, order status changes, marking an order paid and dispute handling are exposed in no mode at all, because FluentCart offers no supported way to reverse them.

`guarded` exists and is accepted, but in 2.0.0 it exposes exactly what `reversible` does. The two tools it was built for are unavailable in this release — the reasoning is in [the deployment guide](/docs/fluentcart-mcp/deployment#refunds-and-cancellations-in-2-0-0), and the short version is that the guard is finished but could not be acceptance-proven against a store that has no way to clean up a test refund.

Then your store gets a say. The routes discovered at startup prune what's left: a tool needs its endpoint served to be registered, and a tool that calls several endpoints needs all of them. Anything your FluentCart build doesn't offer is not registered at all.

Both filters are exposure, not permission-checking at call time. A tool that fails either is missing from every registry: it cannot be listed, searched, described, or invoked by name. Every mode below draws from the same filtered set, so no choice of mode can widen what the write mode and your store's routes allow.

## Modes

`--mode` changes how those definitions are presented to the model, and what that presentation costs in context.

### Dynamic *(the default)*

Five meta-tools. The AI searches, reads a schema, then executes — instead of holding the whole catalogue in context.

| Tool | What It Does |
|------|-------------|
| `fluentcart_search_tools` | Search by keyword and optional category. Returns names, titles, descriptions. |
| `fluentcart_describe_tools` | Full input schemas and annotations for named tools, five per request. |
| `fluentcart_execute_read_tool` | Execute a read-only tool by name. |
| `fluentcart_execute_reversible_write` | Execute a reversible write. Present only when the write mode allows one. |
| `fluentcart_execute_guarded_write` | Reserved for guarded actions. No tool is routed through it in 2.0.0. |
...
A shortlist of the tools most workflows actually reach for, registered up front. No search step, a fraction of the definitions.

### Code

Two tools — `fluentcart_search_api` and `fluentcart_execute_code` — over a QuickJS WebAssembly sandbox. The AI writes a small script that queries the API and returns a result, which beats twelve round trips when it needs to correlate things. The sandbox is read-only by construction: it has no route to a write tool, whatever your write mode says.

### Full

Every exposed definition registered at connection time. Fastest per call, most expensive in context, and the only mode where the exposed count is also the number of definitions your model must hold.

### What each one costs

Each mode has a definition-token budget the build enforces. Not a target anyone aims at — a ceiling that fails the build when crossed, which is what stops the tool list quietly bloating release by release:

| Mode | Budget |
|---|---|
| `dynamic` | 1,500 tokens |
| `code` | 1,200 tokens |
| `curated` | 12,000 tokens |
| `full` | measured and reported, never gated |

Dynamic and code are fixed costs by design: their payload is a handful of meta-tools, so it doesn't grow when your catalogue does. Curated grows slowly. Full grows with everything your write mode exposes, which is the entire argument for the other three.

The actual figures for a given revision come from `node scripts/measure-tool-context.mjs` in the package, serialised with `mcp-tools-list-v1` and counted with `gpt-tokenizer@3.4.0`, and are recorded per profile in `release-contract.json`. They are not reproduced here: a measurement copied into a documentation page is wrong by the next release and nothing tells you.

<Callout type="info" title="Trade-off">
Dynamic mode spends 2-3 extra tool calls per workflow: search, describe, execute. If round-trip latency hurts you more than context does, `full` is there and it is a legitimate choice.
</Callout>
...
<Accordion title="Orientation & Search">

Start here. These answer "what am I connected to and what can I ask" without a fishing expedition through the rest of the list.

| Tool | Method | What It Does |
|------|--------|--------------|
| `fluentcart_get_store_context` | GET | Store origin, name, currency, timezone, component versions, exposed capabilities and the current write mode. Unset values come back as null with a warning rather than a guess |
| `fluentcart_get_search_capabilities` | GET | Which filters this store has actually been proven to accept, per entity |
| `fluentcart_search_commerce` | GET | Search orders, products, customers or subscriptions. Rejects a filter the store hasn't proven, rather than sending it and getting a full unfiltered page back that looks like a narrow result |
| `fluentcart_list_reference_data` | GET | Resolve the identifiers other tools expect — payment methods, tax classes, shipping zones, countries, labels, categories — instead of guessing a slug |

</Accordion>
<Accordion title="Orders — Core">
...
| `fluentcart_order_refund` | POST | **Not available in 2.0.0** — absent from every write mode |
...
<Accordion title="Products — Pricing">
...
Read-only in this release. You can inspect a billing relationship; you cannot change one.
...
| `fluentcart_subscription_cancel` | POST | **Not available in 2.0.0** — absent from every write mode |

Pausing, resuming and reactivating are not exposed as tools. The FluentCart admin still does all three.
...
Key endpoints feature **response transforms** — `order_get`, `product_get`, `customer_get` and all list endpoints strip HTML blobs, embedded objects, and serialised metadata before returning. Less noise, fewer tokens, same useful data.

When a response is too big to be useful even after trimming, the call fails with `RESPONSE_TOO_LARGE` and names the ways to narrow it — a page size, a date range, fewer fields. Earlier versions returned a quietly truncated payload with a `_truncated` marker, which looked like data and read like a lie. An error you can act on beats a half-answer you can't detect.
...
Resources are application-controlled — the client decides when to read them. Each read goes to your store: the resource layer holds no cache of its own, so treat a resource read as an API call, not a free lookup.
...
A handful of read tools opt into a TTL cache so the AI doesn't re-fetch the same 250-country list every time it validates an address. It is opt-in per tool, not a blanket policy:

| Tool | TTL |
|------|-----|
| `fluentcart_misc_countries` | 1 hour |
| `fluentcart_shipping_zone_states` | 1 hour |
| `fluentcart_email_shortcodes` | 1 hour |
| `fluentcart_tax_eu_rates` | 1 hour |
| `fluentcart_app_init` | 10 minutes |
| `fluentcart_misc_filter_options` | 10 minutes |
| `fluentcart_report_meta` | 10 minutes |
| `fluentcart_shipping_zone_list`, `fluentcart_shipping_class_list` | 10 minutes |
| `fluentcart_role_list`, `fluentcart_tax_class_list`, `fluentcart_tax_settings_get` | 10 minutes |

Everything else goes to your store every time. The store of values lives in the server process and dies with it — no Redis, no external dependency, and nothing shared between processes, so an HTTP deployment behind several replicas gets whatever each replica happens to have seen. Resource reads do not use it at all.
