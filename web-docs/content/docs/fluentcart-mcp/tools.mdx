...
description: Every tool in FluentCart MCP, listed by module. 276 tools covering orders, products, customers, subscriptions, shipping, tax, reports, and more.
---
...
The complete inventory. Every tool, every module, every HTTP method. 276 tools organised into 27 modules so the AI (and you) can find what it needs.
...
Don't fancy loading 276 tool definitions into your LLM context? Fair enough. Start the server with `--mode dynamic` and you get 3 meta-tools instead:
...
The AI discovers tools on demand instead of loading all 276 upfront. Same tools, same results — just lazily loaded.
...
| Products (Core + Pricing + Variants + Catalogue) | 52 | Full catalogue management |
...
| **Total (static mode)** | **276** | |
| **Total (dynamic mode)** | **3** | Meta-tools that proxy to all 276 |
