...
Without `FLUENTCART_MCP_API_KEY`, anyone who discovers your endpoint can query your store, create orders, issue refunds — the full 279-tool experience. Thrilling for you, potentially expensive. Set the key.
</Callout>
...
The HTTP transport creates a fresh server instance per request. This keeps things simple and works with scale-to-zero deployments, but it means each request resolves config and registers all 279 tools. On a reasonable VPS, this adds maybe 10-20ms overhead. Not exactly a crisis.
</Callout>
