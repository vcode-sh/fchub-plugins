...
|----------------|------------------|
| "Call fluentcart_orders_list with per_page=10" | "Show me the latest 10 orders" |
...
The AI picks the right tool, fills in the parameters, and calls my server. You'll see it working — most clients show which tools are being called in real time.
...
| Can't Do | Why |
|----------|-----|
| **Charge credit cards** | Payments require customer interaction (redirects, card forms). My server can mark orders as paid, but it can't process payments. |
...
| **Touch other WordPress data** | My server only talks to FluentCart endpoints. It can't read your blog posts, other plugins, or WordPress users. |
