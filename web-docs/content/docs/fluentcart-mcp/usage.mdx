...
A default install reads and nothing else. Setting `FLUENTCART_WRITE_MODE=reversible` adds one specific family of writes: **creating and updating** products, variants, coupons, customers, shipping rules, tax rules, attribute groups and terms, order bumps and upgrade paths. [Advanced configuration explains the switch](/docs/fluentcart-mcp/configuration#wordpress-scope-and-write-mode-are-different-controls).
