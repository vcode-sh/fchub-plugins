---
title: FCHub Stream (Discontinued)
description: Historical documentation for the discontinued FluentCommunity video plugin. Unsupported, with critical maintenance fixes only.
---
...
FCHub Stream was built to address this. Users could upload videos directly to streaming CDNs (Cloudflare Stream or Bunny.net), with transcoding and embedding handled automatically. The documentation below is retained as historical reference, not a promise that the plugin still works with current versions.
