...
No external HTTP calls and no scheduled refresh. Enter one rate for every configured display currency in the admin and save them together. If validation or persistence fails, the previous manual snapshot remains current.
...
Remote-provider refreshes run on a WordPress cron schedule. Manual rates use their explicit save action instead.
...
<Steps>
  ...
  <Step>
    **Validation**
    ...
    The configured display-currency set is the contract. Extra provider rates are ignored, but every configured quote must be present, numeric, and greater than zero. One missing or invalid quote rejects the whole snapshot — because a mixed-age rate table is not a successful refresh wearing a convincing hat.
  </Step>
  <Step>
    **Storage**

    The complete snapshot is inserted into `fchub_mc_rate_history` with one multi-row write and one timestamp. Only after that write succeeds are all corresponding entries replaced in the WordPress object cache (group `fchub_mc_rates`, 1-hour TTL). A failed write leaves the previous cache untouched.
  </Step>
  <Step>
    **Pruning**
...
You can also trigger an immediate remote refresh from the admin: **FluentCart > Settings > Multi-Currency > Exchange Rates > Refresh Now**. This calls the same action as the cron job. Manual mode uses **Save Manual Rates** instead; the generic refresh endpoint rejects it rather than laundering an old rate through a shiny new timestamp.
...
After the exchange rate multiplication, the resulting amount is rounded according
to your settings. These examples use a two-decimal display currency:
...
After decimal rounding, an optional [charm-rounding rule](/docs/fchub-multi-currency/configuration#charm-rounding)
can snap the converted price to a selling ending — `.99`, `.95`, whole
amounts, or the nearest 5 or 10. Off by default.

The display currency's **Decimals** value controls the granularity:

| Decimals | Rounds To | Example (input: 12.3456, Half Up) |
|---|---|---|
| 0 | Nearest whole unit | `12` |
| 2 | Nearest hundredth | `12.35` |
| 3 | Nearest thousandth | `12.346` |
