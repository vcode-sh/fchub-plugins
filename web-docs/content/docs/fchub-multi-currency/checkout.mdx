...
After the server confirms a visitor's choice, the storefront reloads once with that
currency resolved. Every price on the returned page then moves together — product
cards, "From" prices, compare prices, cart totals, pricing tables, the lot. Later
cart, quantity and variant changes are converted in place without another reload.
