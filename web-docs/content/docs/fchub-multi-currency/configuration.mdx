---
title: Configuration
description: Set up currencies, exchange rates, switcher defaults, checkout notices, and CRM sync. More control, same plugin.
---
...
This is the currency your store actually charges in. FluentCart processes payments
in this currency, and every exchange rate is calculated relative to it. Multi-Currency
reads it directly from **FluentCart → Settings → Store Setup**; there is no second
editable base-currency setting here to drift out of date.

<Callout type="warn" title="Think Before Changing">
Switching your base currency after you've been running for a while? Rates saved for
the previous base are ignored immediately, so the storefront safely falls back to
the new base until a complete replacement set exists. Refresh the configured remote
provider, or save a complete set on the Manual provider. Existing orders keep the
base currency and rate captured when they were placed.
</Callout>
...
A `?currency=` link shows the recipient that currency for the page they land on,
without overwriting a preference they had already set — a shared link should not
quietly rewrite somebody else's choice. Switching does not add the parameter: the
browser applies the currency in place, so there is nothing to clean out of the
address bar afterwards. Campaign links stay in the address bar unless they match an already
confirmed browser choice.

### Cookie Persistence

Should the plugin remember which currency a visitor picked? When enabled, their
choice is saved in a browser cookie and mirrored in local storage for the same
configurable lifetime, 90 days by default. They come back next week — still in EUR.
No need to pick again. The cookie contains the three-letter code; the mirror contains
the same code and its expiry time. It exists so the storefront can recover when an
edge host serves one shared page without consulting the cookie.

When disabled, the browser mirror is removed and every page load starts fresh from
the default display currency. Most stores want this **on**.
...
### Detect from Visitor Locale

On a true first visit — no saved cookie, no account preference, no `?currency=`
link — the switcher can start on the currency the visitor's own clock implies:
their browser's timezone maps to a country, the country to one of your offered
currencies. A visitor in Warsaw sees PLN before touching anything, provided you
offer PLN.

<Callout type="info" title="Why this runs in the browser">
The detection never touches your server. The page ships a small map of "which
timezones imply which of your offered currencies" — a store fact, identical for
every visitor, so cached pages stay byte-identical and no geolocation service,
IP lookup, or personal data is involved. The browser reads its own timezone and
picks from the map at paint time.
</Callout>

The hint is deliberately timid: it never outranks anything the visitor chose or
any campaign link, and it is never stored — an explicit switch replaces it
through the normal persistence path. Unmapped timezones and currencies you do
not offer fall through to the default display currency. Off by default.
...
Use the search field at the top to find currencies from FluentCart's own catalogue.
Click one to add it to your list. Multi-Currency does not maintain a competing list:
names, symbols and zero-decimal defaults come from the FluentCart 1.6.1 currency
APIs. Each currency in the table shows:
...
for each currency individually. FluentCart identifies zero-decimal currencies; its
catalogue otherwise defaults to two decimals, so set three or four here when your
display currency needs them. Hit the red **x** to remove one. If it was the default
display currency, the default safely returns to FluentCart's base currency.
...
<Callout type="info" title="The Base Currency Is Automatic">
Once at least one display currency is configured, the switcher includes FluentCart's
base currency automatically. You may keep it in the list to control its order and
presentation, but you do not have to add it merely to let visitors switch back.
</Callout>

After adding or removing currencies, save this page and then refresh the remote
provider or save the complete Manual rate set. A configured currency without a
usable rate cannot be selected; the switcher keeps the existing preference and says
why. An older stored preference whose rate later becomes unusable safely resolves
to the base currency. Optimistic exchange rates are not a feature, despite what the
average crypto dashboard suggests.
...
<Callout type="info" title="Fast QA Setup">
...
After conversion and decimal rounding, optionally snap converted prices to
selling endings — the psychological-pricing step. Display-only, like every
converted number: checkout still charges the base currency exactly, and the
disclosure already calls converted prices approximate.

| Rule | 34.55 becomes | 1722.52 becomes |
|---|---|---|
| No charm rounding | 34.55 | 1722.52 |
| Whole amounts | 35 | 1723 |
| …99 endings | 34.99 | 1722.99 |
| …95 endings | 34.95 | 1722.95 |
| Nearest 5 | 35 | 1725 |
| Nearest 10 | 30 | 1720 |

The rule applies uniformly to every converted surface — items, ranges, compare
prices, variant buttons — because a charmed item next to an uncharmed total
reads as a bug. Zero-decimal currencies collapse the `.99`/`.95` endings to
whole amounts, free items stay free, and discounts stay negative. Off by
default (`No charm rounding`), so an untouched store keeps its exact
conversions.

### Current Rates

The bottom of the tab shows your live rates — base currency, converted currency, the rate, which provider fetched it, when, and whether it's fresh (green **OK**) or stale.

Remote providers show **Refresh Now**. A refresh succeeds only when the provider returns a valid rate for every configured display currency, so a half-updated table never gets to cosplay as current data. Manual mode instead shows one input per configured display currency and **Save Manual Rates**; all values are saved as one complete snapshot.
...
1. **General tab** → Make sure it is **Enabled** and confirm the base currency read from FluentCart. Change it in FluentCart Store Setup if needed.
2. **Currencies tab** → Add the display currencies your customers use, then save. FluentCart's base currency is included automatically.
...
<Accordions>
  <Accordion title="Complete Settings Keys and Defaults (for developers)">
    ...
    | `base_currency` | string | FluentCart store currency | Read-only base transaction currency |
    ...
    | `geo_enabled` | string | `no` | First-visit locale detection (browser-side) |
    | `rate_provider` | string | `manual` | Exchange rate provider |
    ...
    | `charm_rounding` | string | `none` | Charm rounding rule for converted display prices |
