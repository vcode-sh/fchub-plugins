...
For logged-out visitors, a successful switch also reloads once through this
parameter. That lets the server render the selected currency before a shared edge
cache gets a chance to ignore the preference cookie. The browser verifies that
currency through a uniquely cache-busted request, then removes only the one-shot
parameter. Campaign links stay in the address bar unless they match an already
