...
Should the plugin remember which currency a visitor picked? When enabled, their choice is saved in a browser cookie for 90 days (configurable). They come back next week — still in EUR. No need to pick again. The cookie contains only the three-letter currency code and is readable by the storefront recovery script so an edge-cached page can correct itself.
