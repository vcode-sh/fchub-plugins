...
The `fchubMcConfig` object is emitted as typed JSON before the shared context script
...
| `resolverSource` | string | Winning resolver: `url_param`, `user_meta`, `cookie`, `geo`, or `default` |
...
| `allowedCurrencyCodes` | array | Base plus configured display currency codes accepted by browser recovery |
