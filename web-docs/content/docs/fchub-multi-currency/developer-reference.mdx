...
|---|---|---|
| `fchub_mc:context_changed` | `window` | Fired before page reload after a confirmed currency switch |
...
### Cached Guest Recovery

Some edge hosts serve the same anonymous HTML whether the request carries no
currency cookie, `EUR` or `USD`. The browser contract therefore treats the cookie
and local-storage record as two transports for one confirmed guest preference:

1. `POST /context` must return `persisted: true` before the browser mirror is written.
2. A guest reloads through the configured currency URL parameter, so the first
   returned document resolves correctly without the cookie.
3. The explicit currency is revalidated through `GET /context?currency=<code>` with
   a one-use `_fchub_mc` cache-buster. A confirmed switch parameter is removed only
   after that succeeds; unrelated query values, fragments and genuine direct links
   remain.
4. On later shared-cache hits, `currency-context.js` compares the valid, unexpired
   browser mirror with the rendered context and requests
   `GET /context?currency=<code>` with a one-use `_fchub_mc` cache-buster when
   they differ.
5. An explicit URL preference or signed-in `user_meta` context wins over the browser
   mirror. A cookie or mirror changed during recovery also wins over the older
   response.

The local-storage record contains the currency code and expiry timestamp. Legacy
plain-code values are migrated, while expired, malformed and no-longer-configured
currencies are removed. Disabling cookie persistence removes the mirror as well;
it is not a back door around the setting.
