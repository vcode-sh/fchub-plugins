...
query resolves that allowed browser-cookie preference for cached-page recovery,
independently of the public URL-parameter setting. Every response uses
...
authenticated request, user meta. Guest calls omit `X-WP-Nonce`; signed-in calls
include the current REST nonce so WordPress may authenticate the account. A
confirmed write fires `fchub_mc/context_switched`.
...
The code is validated against the base and configured display currencies.
Failures use stable outcome codes and `persisted: false`: malformed JSON returns
400, a disabled module 403, unavailable persistence 409, an invalid currency
422, and rate limiting 429. These responses also use `Cache-Control: no-store`.
