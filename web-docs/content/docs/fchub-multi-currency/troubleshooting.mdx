...
<Accordions>
  ...
  <Accordion title="Prices showing in base currency despite switching">
    ...
    4. **Host strips cookies on uncached paths** (fixed automatically as of 1.4.5) — some hosts run an edge/WAF layer that strips cookies by default on any URL path it hasn't explicitly whitelisted, which a growing site with new landing pages can't keep ahead of. As of 1.4.5 the switcher also mirrors a guest's choice to `localStorage`, and `currency-projection.js` reconciles it against the page on load via `GET /fchub-mc/v1/context?currency=<code>` — which resolves from the URL query string, not the cookie. Check the Network tab for a `context?currency=` request and a `fchub_mc:context_reconciled` event. This requires `js_projection` and cookie persistence (`guestLocalStorageEnabled`) both enabled, and the REST route itself reachable on this host — see [Guest Currency Reconciliation](/docs/fchub-multi-currency/developer-reference#guest-currency-reconciliation) in the developer reference.
  </Accordion>
</Accordions>
...
<Accordions>
  ...
  <Accordion title="Currency switch doesn't stick">
    ...
    3. **Cache serving stale HTML** — full page cache may serve the old page. Clear cache or exclude the cookie. As of 1.4.5, guests get an automatic client-side correction for this — see the "Host strips cookies on uncached paths" item above
  ...
  <Accordion title="No-JS fallback works, but does not remember the selection">
    ...
    4. **Host strips cookies** — the 1.4.5 localStorage/reconciliation fallback is JavaScript-only, so it doesn't help the no-JS form path. A host that strips the cookie will still lose a no-JS guest's selection
  </Accordion>
