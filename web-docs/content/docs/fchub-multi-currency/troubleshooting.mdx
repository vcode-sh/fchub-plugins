...
<Accordions>
  ...
  <Accordion title="Prices showing in base currency despite switching">
    ...
    1. **Cookie blocked** — verify the `fchub_mc_currency` cookie exists in the browser. There is nothing to recover when the browser refuses to store the preference.
    2. **Recovery request blocked** — cached pages are repaired through `GET /fchub-mc/v1/context`, with browser and intermediary caching disabled. Check the Network tab for a failed request.
    3. **Old HTML still cached after an update** — purge the page or edge cache once so it serves the recovery runtime added in 1.4.5. Per-page cache exclusions are not required after that.
  </Accordion>
</Accordions>
...
<Accordions>
  ...
  <Accordion title="Currency switch doesn't stick">
    ...
    3. **Recovery API unavailable** — an edge-cached page may initially contain another currency, but `GET /context` should correct it before projection. A firewall rule blocking WordPress REST requests rather spoils that plan.
