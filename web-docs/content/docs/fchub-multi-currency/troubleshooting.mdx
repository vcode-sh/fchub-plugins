...
<Accordions>
  ...
  <Accordion title="Prices showing in base currency despite switching">
    ...
    4. **Host strips cookies on uncached paths** — some hosts run an edge/WAF layer that strips cookies by default on any URL path it hasn't explicitly whitelisted, which a growing site with new landing pages can't keep ahead of. This is handled automatically for guests: a currency switch itself now reloads through a URL parameter that resolves ahead of the cookie, so it never depends on the cookie surviving that round-trip in the first place. For a guest arriving fresh at some other page (a stale cached page, a bookmark), the switcher separately mirrors the choice to `localStorage`, and `currency-projection.js` reconciles it against the page on load via `GET /fchub-mc/v1/context?currency=<code>` — which also resolves from the URL query string, not the cookie. Check the Network tab for a `context?currency=` request and a `fchub_mc:context_reconciled` event on that path. Both require `js_projection` and cookie persistence (`guestLocalStorageEnabled`) enabled, and the REST route itself reachable on this host — see [Guest Currency Reconciliation](/docs/fchub-multi-currency/developer-reference#guest-currency-reconciliation) in the developer reference.
  </Accordion>
</Accordions>
...
<Accordions>
  ...
  <Accordion title="Currency switch doesn't stick">
    ...
    3. **Cache serving stale HTML** — full page cache may serve the old page. Clear cache or exclude the cookie. Guests get an automatic client-side correction for this — see the "Host strips cookies on uncached paths" item above
  ...
  <Accordion title="No-JS fallback works, but does not remember the selection">
    ...
    4. **Host strips cookies** — the localStorage/reconciliation fallback is JavaScript-only, so it doesn't help the no-JS form path. A host that strips the cookie will still lose a no-JS guest's selection
  </Accordion>
