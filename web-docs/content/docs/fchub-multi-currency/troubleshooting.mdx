...
<Accordions>
  ...
  <Accordion title="Currency switch doesn't stick">
    ...
    1. **Browser persistence blocked** — check that the `fchub_mc_currency` cookie or local-storage mirror is being written. Some privacy plugins or browser settings may block both
    2. **REST API error** — `POST /context` must return `persisted: true`. The switcher deliberately stays put when the server could not save anything.
    3. **Recovery API unavailable** — an edge-cached page may initially contain another currency, but `GET /context?currency=...` should correct it without relying on the cookie. A firewall rule blocking WordPress REST requests rather spoils that plan.
