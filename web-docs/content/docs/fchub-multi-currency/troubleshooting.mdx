...
<Accordions>
  ...
  <Accordion title="Prices showing in base currency despite switching">
    ...
    4. **Host strips cookies on uncached paths** (fixed automatically in a later update) — some hosts run an edge/WAF layer that strips cookies by default on any URL path it hasn't explicitly whitelisted, which a growing site with new landing pages can't keep ahead of. The switcher also mirrors a guest's choice to `localStorage`, and `currency-projection.js` reconciles it against the page on load via `GET /fchub-mc/v1/context?currency=<code>` — which resolves from the URL query string, not the cookie. Check the Network tab for a `context?currency=` request and a `fchub_mc:context_reconciled` event. This requires `js_projection` and cookie persistence (`guestLocalStorageEnabled`) both enabled, and the REST route itself reachable on this host — see [Guest Currency Reconciliation](/docs/fchub-multi-currency/developer-reference#guest-currency-reconciliation) in the developer reference.
  </Accordion>
</Accordions>
