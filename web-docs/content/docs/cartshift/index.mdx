...
<Cards>
  ...
  <Card title="Product Mapping">
    Already rebuilt part of your catalogue in FluentCart by hand? Link each WooCommerce product to the one you made instead of getting a duplicate. Orders, subscriptions and coupons follow the link. Skipped when FluentCart is empty.
  </Card>
...
|---|---|---|
| `/cartshift/v1/preflight` | GET | Run preflight environment checks |
| `/cartshift/v1/counts` | GET | Get WooCommerce entity counts |
| `/cartshift/v1/migrate` | POST | Start the migration |
| `/cartshift/v1/progress` | GET | Get current migration progress |
| `/cartshift/v1/cancel` | POST | Cancel a running migration |
| `/cartshift/v1/rollback` | POST | Rollback migrated data |
| `/cartshift/v1/log` | GET | Retrieve paginated migration log |
...
Ready to migrate? Start with [Installation](/docs/cartshift/installation) to get the plugin set up, then follow the [Migration Guide](/docs/cartshift/migration-guide) for a complete step-by-step walkthrough.
