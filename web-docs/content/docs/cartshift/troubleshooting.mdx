...
<Accordions>
  ...
  <Accordion title="FluentCart already contains data">
    ...
    **Options:**
    ...
    - After migration, use [rollback](/docs/cartshift/rollback) to remove only the migrated records if the result is not what you expected.
  </Accordion>
</Accordions>
...
<Accordions>
  <Accordion title="Migration appears stuck">
    ...
    **Diagnosis:** Check the migration state directly in the database:
    ...
    ```sql
    SELECT option_value FROM wp_options WHERE option_name = 'cartshift_migration_state';
    ```
    ...
    **Recovery:**
    ...
    1. Cancel the migration: `POST /cartshift/v1/cancel`
    2. If cancel does not work (the request may time out too), manually update the option:
       ```sql
       DELETE FROM wp_options WHERE option_name = 'cartshift_migration_state';
       ```
  ...
  <Accordion title="A migration is already running">
    ...
    **Fix:**
    ...
    1. Try cancelling: `POST /cartshift/v1/cancel`
