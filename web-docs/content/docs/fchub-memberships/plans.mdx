...
Plans are the core building block of FCHub Memberships. A plan defines the
offer, how long access lasts, and what the member receives.

<ImageZoom
  src="/memberships/memberships-plans.webp"
  alt="Plans workspace showing plan health, readiness, search, and existing membership offers"
  width={1492}
  height={840}
...
Choose **Memberships > Plans > Create Plan** to open the three-step builder:

<ImageZoom
  src="/memberships/memberships-plan-builder.webp"
  alt="Guided plan builder showing offer details, three clear setup steps, and a live plan summary"
  width={1492}
  height={840}
  className="rounded-xl border my-6"
/>
...
Name the plan, choose its availability, and decide how long access lasts.
Trials, grace periods, membership terms, hierarchy, and restriction behaviour
...
Add WordPress content or supported provider resources and choose whether each
item unlocks immediately, after a delay, or on a fixed date. This step can be
skipped when the offer is not ready for content yet.
...
Confirm availability, duration, content, and trial behaviour before creating
the plan. The builder returns you to the relevant step if something needs
attention instead of merely announcing that somewhere, somehow, a field is
wrong.
...
Plans connect to FluentCart products so a successful purchase can grant the
right access automatically. The quickest route starts inside the saved plan.
...
Edit the membership plan and choose **Linked Products**.
...
Choose **Link product**, search by product or variation name, and select the
offer customers will buy.
...
The plan editor creates the FluentCart membership connection and shows it in
the linked-products list. You can unlink it from the same place later.

</Step>
<Step>

**Use FluentCart feeds for advanced setups**

If a product needs multiple membership feeds or feed-specific overrides, open
the FluentCart product, add a **Memberships** integration, and configure the
plan, validity, cancellation, user creation, and revocation behaviour there.
