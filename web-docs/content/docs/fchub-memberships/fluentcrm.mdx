...
FCHub Memberships connects the full membership lifecycle to FluentCRM. Start
automations when access changes, manage memberships from automation actions,
personalise messages with membership data, and segment contacts by their
current relationship with your plans.

Version 1.4.0 also adds visible provider health and recovery. If a CRM update
falls behind, **Memberships > Integrations** shows the problem and can preview
or repair the difference without asking you to infer production health from a
green toggle.

## Health and Recovery

The Integrations workspace brings together:

- current FluentCRM availability and compatibility;
- pending or failed membership updates;
- contacts whose CRM state has drifted from membership access;
- a safe preview before repair;
- single-member and bounded all-member recovery.

Membership changes remain the source of truth while FluentCRM catches up. A
temporary CRM failure does not roll back valid member access.
...
| Trigger | Hook | When It Fires |
|---|---|---|
| **MembershipExpiringSoon** | `fchub_memberships/grant_expiring_soon` | Membership approaching expiration (daily check) |
