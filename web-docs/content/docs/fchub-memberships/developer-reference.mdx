...
- Add one to ten unique destination URLs. Production destinations require
  HTTPS and must resolve only to public addresses.
- Generate a signing secret. It is shown once; later settings responses expose
  only whether it is configured.
- Send a production-path test, then enable delivery. The server refuses to
  enable an unsafe destination or an unsigned configuration.
...
Secret rotation is refused while any delivery is pending, processing, or
retrying. A terminal failed delivery may be retried after rotation and will be
