...
| GET | `/admin/members/{user_id}` | Get a member's details and memberships |
...
| POST | `/admin/members/pause` | Pause the membership containing the body `grant_id` |
| POST | `/admin/members/resume` | Resume the membership containing the body `grant_id` |
...
| GET | `/admin/members/{user_id}/provider-state` | Classify the member's provider assignments (read-only) |
...
A plan writes one grant row per rule, so a `grant_id` identifies a row, not a
membership. Pause and resume therefore act on every row of the membership the
given grant belongs to. The request shape is unchanged; a caller that pauses one
row no longer leaves the rest of that membership active.

`GET /admin/members/{user_id}` returns `user` and `memberships`. Each membership
carries its derived status, term, resolved source, grant ids, and the resources
it unlocks. Drip progress is read from `/admin/members/{user_id}/drip-timeline`
and provider classification from `/admin/members/{user_id}/provider-state`;
neither is embedded in the profile payload, because the first is only needed on
demand and the second calls the providers themselves. Repairs continue to go
through `POST /admin/provider-reconciliation/repair`.
