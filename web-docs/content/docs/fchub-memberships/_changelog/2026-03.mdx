...
*Released March 2026* — [GitHub Release](https://github.com/vcode-sh/fchub-plugins/releases/tag/fchub-memberships/v1.3.1)
...
- **FluentCart account integration** — a "Memberships" tab appears in the customer portal at `/account/memberships/`, sitting alongside Purchase History and Downloads like it always should have
- **Standalone shortcode** — `[fchub_my_memberships]` now renders a proper Vue app instead of static HTML. Same data, dramatically less depressing
- **Plan dashboard cards** — each plan shows expiry date, days remaining, content progress bar, and a collapsible content library with drip status
