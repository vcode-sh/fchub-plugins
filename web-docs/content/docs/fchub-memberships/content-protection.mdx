...
Content protection decides who can see what and what everyone else sees
instead. Version 1.4.0 adds a guided workflow so the first decision is “what
should I protect?” rather than “which internal resource key sparks joy?”

## Guided Protection Wizard

Go to **Memberships > Content** and choose **Protect content**.

<ImageZoom
  src="/memberships/memberships-protection-wizard.webp"
  alt="Content Protection wizard showing four setup steps and the available WordPress content types"
  width={1492}
  height={840}
...
Browse or search for the exact item. The picker uses friendly labels and keeps
the technical identifiers out of the way.
...
Choose the plans that unlock it and decide whether visitors should see a
message, follow a redirect, or get a teaser.
...
Confirm the resource, audience, and visitor experience, then apply the rule.
...
When a visitor requests protected content, Memberships looks at every valid
source of access before deciding what to show:

- direct access to the resource;
- access through one or more membership plans;
- access inherited from a protected category or taxonomy;
- access that begins later through a drip schedule;
- paused or ended memberships;
- the configurable administrator preview.

If access is not available, the visitor sees the message, redirect, or teaser
you selected. Protected content is also filtered from feeds, archives, search,
and REST responses where applicable, because hiding the page while publishing
the full article elsewhere would be an ambitious interpretation of protection.
