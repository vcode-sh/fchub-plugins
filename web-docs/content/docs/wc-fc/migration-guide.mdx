...
<Steps>
  ...
  <Step>
    **Run preflight checks**
    ...
    Navigate to **Tools > WC-FC Migrator** or call the preflight endpoint directly:
    ...
    All required checks must pass before you proceed. See [Preflight Checks](/docs/wc-fc/preflight) for detailed information about each check and how to fix failures.
  </Step>
  ...
  <Step>
    **Run a dry run (recommended)**
    ...
    ```bash
    curl -X POST "https://yoursite.com/wp-json/wc-fc/v1/migrate" \
      -H "X-WP-Nonce: YOUR_NONCE" \
  ...
  <Step>
    **Monitor progress**
    ...
    ```bash
    curl -X GET "https://yoursite.com/wp-json/wc-fc/v1/progress" \
      -H "X-WP-Nonce: YOUR_NONCE"
    ```
  ...
  <Step>
    **Cancel if needed**
    ...
    ```bash
    curl -X POST "https://yoursite.com/wp-json/wc-fc/v1/cancel" \
      -H "X-WP-Nonce: YOUR_NONCE"
    ```

    The migrator checks for cancellation between every record, so it stops cleanly. Records already migrated before cancellation remain in FluentCart — use [rollback](/docs/wc-fc/rollback) if you want to remove them.
  </Step>
  ...
  <Step>
    **Deactivate the plugin**
    ...
    Once you have verified everything, deactivate WC-FC Migrator. It has served its purpose.

    Go to **Plugins** and click **Deactivate** on "FCHub - WC Migrator". The `wcfc_migration_state` option is cleaned up on deactivation.
...
The `wc_fc_id_map` table is central to the migration. Every time a WooCommerce record is successfully migrated, its WC ID and FC ID are stored along with the entity type.
...
Every processed record generates a log entry in the `wc_fc_migration_log` table:
...
```bash
curl -X GET "https://yoursite.com/wp-json/wc-fc/v1/log?page=1&per_page=50" \
  -H "X-WP-Nonce: YOUR_NONCE"
```
