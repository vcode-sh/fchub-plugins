...
The NIP checksum validation at checkout was wired to the wrong data path. FluentCart passes `{data: formData, cart: cart}` but I read `formData.billing_nip` directly — one level too shallow. Every NIP was accepted. `0000000000`? Sure. `ABCDEFGHIJ`? Why not. Fixed with a proper nested lookup and a fallback for backwards compatibility.
