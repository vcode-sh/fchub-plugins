...
The plugin includes a server-side Polish NIP checksum validation algorithm. Polish NIP numbers are 10 digits with a built-in checksum.
...
When creating an invoice, the `InvoiceHandler` reads these values from the billing address:

```php
$wantsCompanyInvoice = Arr::get(
    $billingAddress->meta ?? [],
    'other_data.wants_company_invoice',
    false
);
...
If both `wants_company_invoice` is truthy and `nip` is non-empty, a B2B invoice is created. Otherwise, it's a B2C invoice.
