...
| `server_error` | KSeF service error | Displayed as "Server Error" in admin. |
...
- **Stops when**: the status changes to any value other than `processing` (e.g., `ok`, `send_error`)
...
The correction flow differs slightly from regular invoices:

1. The correction invoice is created first via the standard `/invoices.json` endpoint
2. After successful creation, the plugin calls the `send_to_ksef` endpoint on the correction invoice
3. The correction's KSeF status is stored separately as `_fakturownia_correction_ksef_status`

This two-step process is necessary because correction invoices reference the original invoice and need to be created before they can be submitted to KSeF.
