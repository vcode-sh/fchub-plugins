...
Navigate to: **FluentCart > Settings > Integrations > Fakturownia**
...
You can also enter the full domain (e.g., `mojafirma.fakturownia.pl`). The plugin handles both formats -- if a dot is detected, it uses the value as-is. If there's no dot, it appends `.fakturownia.pl` automatically.

| Field | Example |
|---|---|
| Subdomain only | `mojafirma` |
| Full domain | `mojafirma.fakturownia.pl` |
