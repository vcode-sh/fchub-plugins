...
Harder than it should be, as it turns out. But not impossible — once you understand the architecture, the boot sequence, and the one place where FluentCart actively sabotages your work (I'll get to that).
...
<Tabs items={['PHP Hooks', 'Sidebar Menu', 'Vue Route', 'The More Menu (The Hard Bit)']}>
  ...
  <Tab value="The More Menu (The Hard Bit)">
    ...
    **The solution: client-side DOM injection.** The menu is server-rendered HTML, so I wait for it to appear in the DOM and surgically append my item:
...
<Accordions type="single">
  ...
  <Accordion title="Cache busting with filemtime() is essential">
    Use `filemtime()` as the version parameter for `wp_enqueue_script`, not your plugin's version constant. The version constant doesn't change between file edits during development, which means the browser (and Cloudflare, if you're using a tunnel) serves stale JavaScript while you stare at unchanged behaviour wondering what's gone wrong. Ask how I know.
  </Accordion>
