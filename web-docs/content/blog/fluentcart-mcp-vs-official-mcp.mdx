...
FluentCart MCP is an independent project; it is not the official server, an adapter for it, or a claim about FluentCart's roadmap. The useful question is not which logo wins a cage match. It is which boundary fits the work in front of you.
...
|---|---|---|
| **Where it lives** | In the FluentCart and WordPress ecosystem | Beside the shop as a separately configured MCP service |
...
For the current measured Core plus Pro profiles, the default dynamic surface has **three** read-only tools: search, describe, and execute a permitted read. The corresponding reversible profile has **four**: it adds the reversible executor. Those are profile measurements, not a promise that every store, role, or configuration sees the same thing.
...
In reversible mode, the server can handle the routine changes that meet its policy. That mode is off by default. The [setup guide](/docs/fluentcart-mcp/setup) explains the configuration; the release contract records the exact measured profile behind the claim.
...
You can use both. Give them different server names and let each keep its own promises. Two MCP servers become confusing only when someone insists they are the same product. They are not, and that is rather the point.
