...
(() => {
	...
	/**
	 * Strips the currency query param back out of the visible URL once this
	 * page's own resolution is done with it, via history.replaceState() —
	 * purely client-side, no navigation, no flicker.
	 *
	 * Only relevant when cfg.source is "url_param": currency-switcher.js's
	 * buildReloadUrl() appends this param to the post-switch reload URL so
	 * UrlParamResolver (Priority 1 in the resolver chain, above the cookie
	 * and above a signed-in visitor's saved preference) resolves the correct
	 * currency in that one request — no second reconciliation fetch needed
	 * (RECONCILABLE_SOURCES excludes url_param, so reconciliationCandidate is
	 * always false here; see below). Left alone, the param would stay in the
	 * address bar indefinitely and give a CDN in front of the page a reason
	 * to treat every currency as a separate cacheable URL.
	 *
	 * Removing it here doesn't risk losing the choice on a later page load:
	 * the switch's own POST already persisted it to the cookie (and, for a
	 * signed-in visitor, user meta) before this reload happened, same as
	 * before this file's reconciliation fallback existed. If that cookie
	 * write didn't stick — the original issue #72 symptom — the localStorage
	 * reconciliation above is exactly the safety net for that on whatever
	 * page the visitor lands on next.
	 */
	function stripUrlParamFromAddressBar() {
		if (cfg.source !== "url_param" || !cfg.urlParamKey) return;
		try {
			const url = new URL(window.location.href);
			if (!url.searchParams.has(cfg.urlParamKey)) return;
			url.searchParams.delete(cfg.urlParamKey);
			window.history.replaceState(window.history.state, "", url.toString());
		} catch {
			// Cosmetic only — never let this block price projection or anything else.
		}
	}
