...
* Fixed prices going fully blank for the entire multi-second reconciliation wait (previously hidden the same way as the plain, non-reconciliation price-conversion FOUC prevention) — a blank gap right after the page finishes reloading reads as "finished" when reconciliation has actually just started. Prices are no longer touched at all — no hiding, no dimming, no other visual treatment — while a reconciliation is in progress; they stay exactly as the page originally rendered them until the moment reconciliation completes, then switch directly to the corrected, converted values in one step. The plain (non-reconciliation) FOUC-prevention path, where prices are hidden briefly while first converted on a normal page load, is unchanged.
* Fixed a currency switch looking visibly different for logged-out visitors than for signed-in ones on hosts where reconciliation actually runs: the switcher's trigger flashed from its on-pick highlighted color to plain white partway through the wait, then back, where a signed-in switch shows no such flash. Root cause, confirmed via direct DOM/CSS inspection: the highlighted look right after picking a currency is the host theme's own `:focus` styling, applied because the trigger is refocused for accessibility just before the page reloads — not anything this plugin colors itself. Reloading the page always clears that focus, identically for every visitor, so the highlight is already gone the instant the fresh page repaints; a signed-in visitor's reload has nothing left to wait for and never shows otherwise. A previous round of this fix had the trigger re-apply and hold its own dimmed/loading look for the length of a logged-out visitor's extra reconciliation wait, which was the only thing making that ordinary, otherwise-instant focus loss visible as a stuck/flashing trigger. That loading state has been removed entirely — the trigger is left alone throughout the reconciliation wait, exactly as it already was for a signed-in visitor's reload, and shows only the server-rendered value until reconciliation completes and (if needed) corrects it directly to the final one.
