...
* Fixed prices going fully blank for the entire multi-second reconciliation wait (previously hidden the same way as the plain, non-reconciliation price-conversion FOUC prevention) — a blank gap right after the page finishes reloading reads as "finished" when reconciliation has actually just started. Prices are now projected immediately with whatever (possibly stale) values the page was served with, dimmed the same way the switcher trigger already is, and swapped directly to the corrected values in one atomic transition once reconciliation completes — never an intermediate blank or raw state. The plain (non-reconciliation) FOUC-prevention path, where prices are hidden briefly while first converted on a normal page load, is unchanged.
