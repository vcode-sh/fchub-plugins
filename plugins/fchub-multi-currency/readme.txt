...
* Fixed logged-out visitors seeing their currency choice revert to the default on hosts whose edge/WAF layer strips the guest currency cookie on request paths it hasn't whitelisted, or whose full-page cache serves a shared, anonymous-visitor snapshot of the page. The switcher now also mirrors a guest's choice to localStorage, and the storefront script reconciles it against the page's baked-in currency on load — via a `GET /context` request carrying the site's configured URL-parameter key (`currency` by default, customizable) that resolves through the URL query string rather than the cookie — correcting client-side whenever the two disagree. Bounded by a 12-second timeout tuned to real host latency, with a matching CSS fallback that never force-reveals prices before that timeout has actually had a chance to resolve them. Disabled automatically alongside cookie persistence, so sites that intentionally turned it off for guests are unaffected, and only attempted at all when URL-parameter resolution itself is enabled — reconciliation depends entirely on that resolver being in the chain, so it's skipped outright rather than fetched and left to time out for nothing when a site has turned it off. Never overrides an explicit `?currency=` link or a signed-in visitor's saved preference.
