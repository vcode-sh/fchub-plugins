...
* Added temporary, opt-in (`?_debug_timing=1`) request-timing instrumentation to `GET /context` for the ongoing issue #72 latency investigation. Not a permanent addition — remove `app/Support/Profiler.php` and its call sites once the bottleneck is found and fixed.
