...
* Fixed the switcher's trigger label displaying a currency the visitor didn't choose while a client-side reconciliation is still in progress. Prices were already correctly hidden during this window, but the trigger itself is server-rendered with whatever currency the page was served with — exactly the value reconciliation exists to double-check — and showed it, uncorrected, for the full multi-second window. It now stays in the same dimmed, non-interactive loading state already used the moment a visitor picks a new currency, until reconciliation settles.
