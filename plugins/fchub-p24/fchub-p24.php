 ...
 * Plugin URI: https://fchub.co/docs/fchub-p24
...
define('FCHUB_P24_VERSION', '1.0.4');
...
add_action('template_redirect', function () {
    ...
    // phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only receipt routing.
    if (
        empty($_GET['trx_hash'])
        || empty($_GET['method'])
        || sanitize_key(wp_unslash($_GET['method'])) !== 'przelewy24'
    ) {
        return;
    }

    $trxHash = sanitize_text_field(wp_unslash($_GET['trx_hash']));
        ...
        if (is_array($config) && isset($config['p24_session_id']) && $config['p24_session_id'] === $trxHash) {
            ...
            if ($currentTransaction) {
                $redirectUrl = add_query_arg(
                    'trx_hash',
                    rawurlencode($currentTransaction->uuid),
                    home_url('/')
                );
