(function () {
    ...
    var root = document.getElementById('wc-fc-app');
    if (!root) return;

    var restUrl = wcfc.restUrl;
    var nonce = wcfc.nonce;
    ...
    function pageHeader(title) {
        var html = '<div class="wcfc-page-header">';
        html += '<h1>' + title + '</h1>';
        html += '<div class="wcfc-theme-switcher">';
        html += '<button class="wcfc-theme-btn" id="wcfc-theme-btn" title="Theme: ' + themeMode + '">' + THEME_ICONS[themeMode] + '</button>';
        html += '<div class="wcfc-theme-dropdown" id="wcfc-theme-dropdown" style="display:none;">';
        html += '<button class="wcfc-theme-option' + (themeMode === 'light' ? ' active' : '') + '" data-theme="light">' + THEME_ICONS.light + ' Light</button>';
        html += '<button class="wcfc-theme-option' + (themeMode === 'dark' ? ' active' : '') + '" data-theme="dark">' + THEME_ICONS.dark + ' Dark</button>';
        html += '<button class="wcfc-theme-option' + (themeMode === 'system' ? ' active' : '') + '" data-theme="system">' + THEME_ICONS.system + ' System</button>';
        html += '</div></div></div>';
        return html;
    }
    ...
    root.addEventListener('click', function (e) {
        var btn = e.target.closest('#wcfc-theme-btn');
        if (btn) {
            e.stopPropagation();
            var dd = document.getElementById('wcfc-theme-dropdown');
            if (dd) dd.style.display = dd.style.display === 'none' ? 'block' : 'none';
            return;
        }
        var opt = e.target.closest('.wcfc-theme-option');
        if (opt) {
    ...
    function renderPreflight() {
        var html = pageHeader('WC-FC Migrator');
        ...
        if (!state.preflight) {
            html += '<button class="button button-primary" id="wcfc-run-preflight">Run Preflight Checks</button>';
            root.innerHTML = html;
            document.getElementById('wcfc-run-preflight').addEventListener('click', runPreflight);
            return;
        }
        ...
        html += '<table class="widefat striped wcfc-checks">';
        html += '<thead><tr><th>Check</th><th>Status</th><th>Details</th></tr></thead><tbody>';

        for (var key in checks) {
            var c = checks[key];
            var icon = c.pass ? '<span class="wcfc-pass">&#10003;</span>' : '<span class="wcfc-fail">&#10007;</span>';
            if (c.warning) icon = '<span class="wcfc-warn">&#9888;</span>';
            html += '<tr>';
        ...
        if (state.preflight.ready) {
            ...
            html += '<button class="button button-primary button-hero" id="wcfc-proceed">Proceed to Migration</button> ';
            html += '<button class="button" id="wcfc-recheck">Re-run Checks</button>';
            html += '</p>';
        } else {
            html += '<div class="notice notice-error inline"><p>Please resolve the failing checks above before migrating.</p></div>';
            html += '<p><button class="button" id="wcfc-recheck">Re-run Checks</button></p>';
        }

        root.innerHTML = html;

        var proceedBtn = document.getElementById('wcfc-proceed');
        if (proceedBtn) proceedBtn.addEventListener('click', function () { state.screen = 'select'; render(); });
        var recheckBtn = document.getElementById('wcfc-recheck');
    ...
    function renderSelect() {
        ...
        for (var i = 0; i < entities.length; i++) {
            ...
            html += '<tr>';
            html += '<td><input type="checkbox" class="wcfc-entity-cb" data-entity="' + e.key + '"' + checked + disabled + '></td>';
        ...
        html += '<div class="wcfc-option-box">';
        html += '<label><input type="checkbox" id="wcfc-dry-run"> <strong>Dry run</strong> &mdash; validate data mapping without writing to FluentCart. Shows what would be migrated.</label>';
        html += '</div>';

        html += '<p style="margin-top:15px;">';
        html += '<button class="button button-primary button-hero" id="wcfc-start">Start Migration</button> ';
        html += '<button class="button" id="wcfc-back-preflight">Back</button>';
        html += '</p>';

        root.innerHTML = html;

        // Bind checkboxes
        var cbs = document.querySelectorAll('.wcfc-entity-cb');
        for (var j = 0; j < cbs.length; j++) {
            cbs[j].addEventListener('change', function () {
                state.selectedEntities = [];
                var all = document.querySelectorAll('.wcfc-entity-cb:checked');
        ...
        document.getElementById('wcfc-start').addEventListener('click', startMigration);
        document.getElementById('wcfc-back-preflight').addEventListener('click', function () { state.screen = 'preflight'; render(); });
    }
    ...
    function renderProgress() {
        ...
        if (state.error) {
            html += '<div class="notice notice-error"><p>' + escHtml(state.error) + '</p></div>';
            html += '<p><button class="button" id="wcfc-back-from-error">Back to Start</button></p>';
            root.innerHTML = html;
            var backBtn = document.getElementById('wcfc-back-from-error');
        ...
        html += '<div class="wcfc-status-bar">';
        html += '<strong>Status:</strong> <span class="wcfc-badge wcfc-badge-' + p.status + '">' + capitalize(p.status) + '</span>';
        if (p.dry_run) html += ' <span class="wcfc-badge wcfc-badge-dryrun">DRY RUN</span>';
        if (p.started_at) html += ' | Started: ' + p.started_at;
        html += '</div>';

        if (p.entities) {
            html += '<table class="widefat striped wcfc-progress-table"><thead><tr><th>Entity</th><th>Progress</th><th>Processed</th><th>Skipped</th><th>Errors</th></tr></thead><tbody>';
            ...
            for (var entity in p.entities) {
                ...
                html += '<td><div class="wcfc-progress-bar"><div class="wcfc-progress-fill wcfc-progress-' + e.status + '" style="width:' + pct + '%;"></div><span class="wcfc-progress-text">' + pct + '%</span></div></td>';
                html += '<td>' + e.processed + ' / ' + e.total + '</td>';
                html += '<td>' + e.skipped + '</td>';
                html += '<td>' + (e.errors > 0 ? '<span class="wcfc-fail">' + e.errors + '</span>' : '0') + '</td>';
                html += '</tr>';
            }
        ...
        if (p.status === 'running') {
            html += '<p style="margin-top:15px;"><button class="button button-secondary" id="wcfc-cancel">Cancel Migration</button></p>';
        } else {
            html += '<p style="margin-top:15px;">';
            html += '<button class="button button-primary" id="wcfc-view-results">View Results & Log</button>';
            html += '</p>';
        }

        root.innerHTML = html;

        var cancelBtn = document.getElementById('wcfc-cancel');
        if (cancelBtn) cancelBtn.addEventListener('click', cancelMigration);

        var resultsBtn = document.getElementById('wcfc-view-results');
    ...
    function renderResults() {
        ...
        if (state.progress && state.progress.entities) {
            ...
            for (var entity in state.progress.entities) {
                ...
                html += '<td>' + e.skipped + '</td>';
                html += '<td>' + (e.errors > 0 ? '<span class="wcfc-fail">' + e.errors + '</span>' : '0') + '</td>';
                html += '<td><span class="wcfc-badge wcfc-badge-' + e.status + '">' + capitalize(e.status) + '</span></td>';
                html += '</tr>';
            }

            html += '<tr class="wcfc-total-row"><td><strong>Total</strong></td><td><strong>' + totalProcessed + '</strong></td><td><strong>' + totalSkipped + '</strong></td><td><strong>' + totalErrors + '</strong></td><td></td></tr>';
            html += '</tbody></table>';
        }
        ...
        if (state.log && state.log.length > 0) {
            html += '<div class="wcfc-log-container">';
            ...
            for (var i = 0; i < state.log.length; i++) {
                var l = state.log[i];
                html += '<tr class="wcfc-log-' + l.status + '">';
                html += '<td>' + escHtml(l.created_at) + '</td>';
                html += '<td>' + escHtml(l.entity_type) + '</td>';
                html += '<td>' + (l.wc_id || '-') + '</td>';
                html += '<td><span class="wcfc-badge wcfc-badge-' + l.status + '">' + capitalize(l.status) + '</span></td>';
            ...
            // Pagination
            if (state.logPages > 1) {
                html += '<p class="wcfc-log-pagination">';
                if (state.logPage > 1) {
                    html += '<button class="button" id="wcfc-log-prev">Previous</button> ';
                }
                html += 'Page ' + state.logPage + ' of ' + state.logPages;
                if (state.logPage < state.logPages) {
                    html += ' <button class="button" id="wcfc-log-next">Next</button>';
                }
                html += '</p>';
            }
        } else {
        ...
        html += '<h2>Actions</h2>';
        html += '<p>';
        html += '<button class="button" id="wcfc-new-migration">New Migration</button> ';
        html += '<button class="button button-link-delete" id="wcfc-rollback">Rollback Migration</button>';
        html += '</p>';

        root.innerHTML = html;

        var prevBtn = document.getElementById('wcfc-log-prev');
        if (prevBtn) prevBtn.addEventListener('click', function () { state.logPage--; loadLog(); });
        var nextBtn = document.getElementById('wcfc-log-next');
        if (nextBtn) nextBtn.addEventListener('click', function () { state.logPage++; loadLog(); });
        var newBtn = document.getElementById('wcfc-new-migration');
        ...
        var rollbackBtn = document.getElementById('wcfc-rollback');
    ...
    function startMigration() {
        ...
        var dryRunEl = document.getElementById('wcfc-dry-run');
