...
for (const viewport of viewports) {
  ...
  test(`renders operator provider cards without overflow at ${viewport.name} width`, async ({ page }) => {
    await page.setViewportSize({ width: viewport.width, height: viewport.height })
    ...
    await expect(health.locator('.provider-card')).toHaveCount(4)
    await expect(health).toContainText('FluentCommunity')
    await expect(health).not.toContainText('FluentCommunity Pro')
    await expect(health).toContainText('Healthy')
    await expect(health).toContainText('LearnDash')
    await expect(health).toContainText('Not yet verified')
    await expect(health).toContainText('Needs attention')
    await expect(health).toContainText('Inactive')
    await expect(health.getByRole('link', { name: 'Review provider' })).toHaveCount(3)
    await expect(health.getByRole('link', { name: 'Review provider' }).first()).toHaveAttribute('href', '#/settings')
    expect(await health.evaluate((node) => node.scrollWidth <= node.clientWidth)).toBe(true)
  })
}
