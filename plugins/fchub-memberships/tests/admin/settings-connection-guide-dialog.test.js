...
describe('settings connection guide dialog', () => {
  ...
  it('explains that webhook destinations belong to the receiver and freezes the delivery contract', () => {
    ...
    expect(wrapper.text()).toContain('Save the destination')
    expect(wrapper.text()).toContain('Generate the signing secret')
    expect(wrapper.text()).toContain('Send a test')
    expect(wrapper.text()).toContain('Enable delivery')
