...
class MembershipResumedEmail
{
    private const SETTING_KEY = 'email_membership_resumed';
    private const TEMPLATE_KEY = 'membership_resumed';

    /**
     * Send the membership resumed notification.
     *
     * @param int   $userId WordPress user ID.
     * @param array $data {
     *     @type string $plan_title Plan name.
     *     @type string $expires_at Membership expiry date (Y-m-d H:i:s).
     * }
     */
    public function send(int $userId, array $data): void
    {
        ...
        if (!$user) {
            Logger::error('MembershipResumedEmail: user not found', "User ID: {$userId}");
            return;
        }
