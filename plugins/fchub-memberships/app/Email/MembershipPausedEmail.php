...
class MembershipPausedEmail
{
    private const SETTING_KEY = 'email_membership_paused';
    private const TEMPLATE_KEY = 'membership_paused';

    /**
     * Send the membership paused notification.
     *
     * @param int   $userId WordPress user ID.
     * @param array $data {
     *     @type string $plan_title Plan name.
     *     @type string $reason     Pause reason (optional).
     * }
     */
    public function send(int $userId, array $data): void
    {
        ...
        if (!$user) {
            Logger::error('MembershipPausedEmail: user not found', "User ID: {$userId}");
            return;
        }

        $smartCodes = $this->buildSmartCodes($user, $data);
        $subject = $this->replaceSmartCodes(
            __('Your {plan_name} membership has been paused', 'fchub-memberships'),
            $smartCodes
        );
    ...
    {
        return <<<'HTML'
<h2>Your membership has been paused, {user_name}</h2>
