...
class MembershipRenewedTrigger extends BaseTrigger
{
    ...
    public function getTrigger()
    {
        return [
            ...
            'label'       => __('Membership Renewed', 'fchub-memberships'),
            'description' => __('This will start when a membership grant is renewed', 'fchub-memberships'),
        ];
    }
    ...
    {
        return [
            'title'     => __('Membership Renewed', 'fchub-memberships'),
            'sub_title' => __('This will start when a membership grant is renewed', 'fchub-memberships'),
            'fields'    => [
                'subscription_status'      => [
    ...
    public function getConditionFields($funnel)
    {
        return [
            ...
            'min_renewal_count' => [
                'type'        => 'input-number',
                'label'       => __('Minimum Renewal Count', 'fchub-memberships'),
                'help'        => __('Only trigger when the renewal count reaches at least this number', 'fchub-memberships'),
                'placeholder' => '1',
            ],
            'run_multiple'      => [
                'type'        => 'yes_no_check',
    ...
    public function handle($funnel, $originalArgs)
    {
        ...
        if (!$this->isProcessable($funnel, $user, $planId, $renewalCount)) {
            return false;
        }
    ...
    private function isProcessable($funnel, $user, $planId, $renewalCount)
    {
        ...
        $minCount = (int) Arr::get($conditions, 'min_renewal_count', 1);
        if ($renewalCount < $minCount) {
            return false;
        }
