...
(function () {
	...
	var UiState = {
		...
		_labels: function (btn) {
			var i18n =
				(window.fchubWishlistVars && window.fchubWishlistVars.i18n) || {};
			var addLabel =
				btn.getAttribute("data-label-add") || i18n.add || "Add to Wishlist";
			var removeLabel =
				btn.getAttribute("data-label-remove") ||
				i18n.remove ||
	...
	var ApiClient = {
		...
		_url: function (endpoint) {
			var base =
				(window.fchubWishlistVars && window.fchubWishlistVars.restUrl) ||
				"/wp-json/fchub-wishlist/v1/";
	...
	function _notifyError(error) {
		var message = _errorMessage(error);

		document.dispatchEvent(
			new CustomEvent("fchub_wishlist:error", {
		...
		_toggle: function (productId, variantId) {
			...
			ApiClient.toggle(productId, variantId)
				.then(function (res) {
					...
					CounterSync.update(
						data.count != null ? data.count : UiState.getCount(),
					);
				})
	...
	var PageHandler = {
		...
		_pushToCart: function (cartItemId) {
			...
			var ajaxUrl =
				(window.fchubWishlistVars && window.fchubWishlistVars.ajaxUrl) ||
				"/wp-admin/admin-ajax.php";
			var cartAjax =
				(window.fchubWishlistVars && window.fchubWishlistVars.cartAjax) || {};
			var action = cartAjax.action || "fluent_cart_checkout_routes";
			var checkoutAction =
				cartAjax.checkout_action || "fluent_cart_cart_update";
	...
	var VariantSync = {
		init: function () {
			...
			function syncButtonsForProduct(productId) {
				...
				for (var i = 0; i < buttons.length; i++) {
					var vid =
						parseInt(buttons[i].getAttribute("data-variant-id"), 10) || 0;
					...
					var observer = new MutationObserver(function () {
						...
						if (
							document.querySelector(
								'[data-fchub-wishlist-toggle][data-product-id="' +
									productId +
									'"]',
							)
						) {
