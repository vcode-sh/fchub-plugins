...
export function orderBumpTools(client: FluentCartClient): ToolDefinition[] {
	return [
		...
		getTool(client, {
			...
			description:
				'Get order bump details: config, discount, conditions, and variant. Requires Order Bumps module to be enabled.',
			schema: z.object({
		...
		postTool(client, {
			...
			description:
				'Create an order bump. Requires title and src_object_id (product variation ID). Requires Order Bumps module to be enabled.',
			schema: z.object({
		...
		putTool(client, {
			...
			description:
				'Update an order bump. Only provided fields are changed. Requires Order Bumps module to be enabled.',
			schema: z.object({
		...
		deleteTool(client, {
			...
			description:
				'Permanently delete an order bump. Cannot be undone. Requires Order Bumps module to be enabled.',
			schema: z.object({
