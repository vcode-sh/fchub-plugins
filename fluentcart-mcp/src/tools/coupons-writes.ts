...
/**
 * Coupon mutations, including applying and cancelling a coupon against an order.
 *
 * Split from coupons.ts, which keeps the read side.
 */
export function couponWriteTools(client: FluentCartClient): ToolDefinition[] {
	return [
		postTool(client, {
		...
		createTool(client, {
			...
			routes: composite(op('GET', '/coupons/{param}'), op('PUT', '/coupons/{param}')),
			title: 'Update Coupon',
			description:
		...
		createTool(client, {
			...
			routes: direct('DELETE', '/coupons/{param}'),
			title: 'Delete Coupon',
		...
		createTool(client, {
			...
			routes: composite(op('GET', '/orders/{param}'), op('POST', '/coupons/apply')),
			title: 'Apply Coupon',
			description:
		...
		createTool(client, {
			...
			routes: composite(op('GET', '/orders/{param}'), op('POST', '/coupons/cancel')),
			title: 'Cancel Coupon',
			description:
